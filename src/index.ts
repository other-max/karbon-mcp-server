...
const isValidGetClientByIdArgs = (args: any): args is { client_id: string; client_type: string } => {
...
const isValidSearchClientsArgs = (args: any): args is { search_term: string; max_results?: number } => {
  return typeof args === 'object' &&
         args !== null &&
...
const isValidGetWorkItemsArgs = (args: any): args is { 
  client_key?: string; 
  work_type?: string; 
  title_filter?: string; 
  max_results?: number 
...
const isValidGetWorkItemByIdArgs = (args: any): args is { work_item_key: string } => {
  return typeof args === 'object' &&
         args !== null &&
...
class KarbonServer {
      ...
      (error) => {
        if (error.response?.status === 429) {
          ...
          throw new McpError(
            ...
            `Karbon API rate limit exceeded: ${error.response?.data?.message || error.message}`
          );
        }
        if (error.response?.status === 401) {
          throw new McpError(
            ErrorCode.InternalError,
            'Karbon API authentication failed. Please check your credentials.'
          );
        }
        if (error.response?.status === 404) {
          throw new McpError(
            ErrorCode.InvalidRequest,
            `Resource not found: ${error.response?.data?.error?.message || error.message}`
          );
        }
        throw new McpError(
          ErrorCode.InternalError,
          `Karbon API error: ${error.response?.data?.error?.message || error.message}`
        );
      }
    );
  ...
  private async handleGetClientById(args: unknown) {
    if (!isValidGetClientByIdArgs(args)) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Invalid arguments for get_client_by_id. Required: client_id (string), client_type (Contact|Organization|ClientGroup)'
      );
    }
  ...
  private async handleSearchClients(args: unknown) {
    if (!isValidSearchClientsArgs(args)) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Invalid arguments for search_clients. Required: search_term (string), optional: max_results (number)'
      );
    }
  ...
  private async handleGetWorkItems(args: unknown) {
    if (!isValidGetWorkItemsArgs(args)) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Invalid arguments for get_work_items. All parameters are optional: client_key, work_type, title_filter, max_results'
      );
    }
    ...
    try {
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Failed to get work items: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }

  private async handleGetWorkItemById(args: unknown) {
    if (!isValidGetWorkItemByIdArgs(args)) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Invalid arguments for get_work_item_by_id. Required: work_item_key (string)'
      );
    }
    ...
    try {
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Failed to get work item: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
